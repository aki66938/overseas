...
Implementation committed as **039de5b — feat(windows): integrate Flutter shell and tray**. Final implementer status: **DONE_WITH_CONCERNS** (explicit native manual acceptance and request-correlation limits above). The scoped commit includes code, tests, and the appended report; no production service/network mutation was performed.

### Task 7 review follow-up: explicit tray intent (base163c0d6)

Addressed the Important P2 in `task-7-review-findings.md`: TrackPopupMenu processes status updates while the visible command remains unchanged. The previous generic callback could turn a displayed disconnect into connect (or vice versa). The tray model now supplies an explicit connect/disconnect/restore command. Native code captures that command and enabled state when the menu opens, rejects a selection if it is now incompatible or disabled, and sends the captured command to Dart. The shared page action owner checks compatibility again against its latest status while preserving busy/polling/uncertainty guards, then performs only the matching operation. Unknown or untyped shell actions cannot fall back to a generic toggle.

RED (in `apps/regen_access`, process `CI=true`): `C:/Users/Eleme/codex_workspace/.tools/flutter-3.47.2/bin/flutter.bat test test/tray_state_test.dart` -> **5 passed, 2 failed**. Both new tests reproduced the bug: stale connected menu after idle caused connects=1 instead of0; stale idle menu after connected caused disconnects=1 instead of0. An earlier invocation used an invalid duplicated working-directory path and did not run tests; it is not RED evidence.

GREEN: `flutter test test/tray_state_test.dart test/access_client_test.dart test/lifecycle_test.dart` -> **35 tests passed**. New regressions reject both stale directions; final assertions also show a fresh compatible command still executes, and the existing explicit-command test verifies shared ownership and uncertainty gating. State-model assertions cover connect, disconnect, and restore semantics. Native standalone rebuild with the previously recorded CMake executable (`--build build/native-tests --config Release`) and `build/native-tests/Release/native_shell_test.exe` -> **34 native assertions passed**; only fake named pipes and owned windows were used.

Final full `flutter test` -> **76 tests passed**. `flutter analyze` -> **No issues found (5.9s)**. Release build output to collect below. Only the four touched Dart files were formatted; no unrelated newline changes or Git configuration changes. `git diff --check` passed with the repository's existing LF→CRLF warnings; output is not described as pristine.

Self-review: captured native intent cannot be replaced by a newer command, native guard handles menu-time updates, Dart guard handles updates racing delivery, and no asynchronous gap exists between the final action check and claiming shared ownership. The two directions have actual failing-then-passing widget regressions; native OS popup interaction itself remains part of the already recorded manual acceptance gate. This follow-up performs no production network tests, Explorer restart, startup setting change, or screenshot capture. Other native/manual acceptance and protocol-correlation limitations remain unchanged.

Final release build: `flutter build windows --release` -> **success (24.5s)**, `build/windows/x64/runner/Release/regen_access.exe`. Follow-up implementation and covering checks are complete; ready for re-review under commit subject `fix(windows): preserve tray menu operation intent`.
