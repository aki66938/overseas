...
### Slice 2: Linux socket, protected files and renderer

- `1437603`: root-owned runtime directory 0750, socket 0660, explicit bind/chown/chmod before listen, no stale-path unlink, inode-checked cleanup, bounded concurrent connections, cancellation and kernel SO_PEERCRED administrator authority. Linux default client verifies both socket metadata and server UID0.
- `a620ed0`: openat/O_NOFOLLOW protected-file walk validates every component's root ownership and absence of group/other write, regular final files, single links and optional pinned SHA256; returns the verified open descriptor. Official core and libcronet hashes are constants from the approved verified archive. Not yet composed into core launch.
- Socket RED: exact `63a9f0d`, local GOOS=linux CGO_ENABLED=0 `go test -c ./internal/platform/linux` failed undefined newSocketServer. Implementation cross-compiled at1437603; native tests are pending parent results. Tests use only private temporary socket/files. Later ordinary-peer fixture copies the test executable into root:65534 0750 temp directory and launches that helper with UID/GID65534, testing actual kernel credentials without creating accounts.
- Trust RED: GOOS=linux CGO_ENABLED=0 `go test -c ./internal/platform/linux` failed undefined openTrustedAt before production helper existed. Implementation cross-compiled; native root file fixtures pending.
- Renderer RED: `go test ./internal/singconfig -run TestLinuxPlatformOnlyChangesTUNName -count=1` failed missing Platform field. GREEN: package passed; test asserts Linux output differs from Windows only by the empirically required interface name, rejecting unknown platform options.
- Client endpoint RED: parent ran exact3b8bdf6 on VM116; archive SHA256 `9d327c4f8edab08b140e621f16125bac9595df4a8c5c43de0c8daae4145dc6e6`. `TestLinuxV1UsesFixedSocketEndpoint` failed because status dialled Windows PipeName (package0.010s exit1).
- `9007706`: Linux v1 now uses `/run/regen-access/control.sock`; Windows and other-platform endpoints retain prior values. Windows `go test ./internal/clientapi ./cmd/regen-access -count=1` passed; Linux clientapi cross-compile passed. Native GREEN pending parent staging.
- No service/network mutation or remote execution by implementer. Cgroupv2 generation ownership and DNS firewall integration design explicitly approved by parent; kernel nft cgroup expression probe pending. NetworkManager without resolved requires isolated runtime restoration evidence before its implementation can be considered accepted.
