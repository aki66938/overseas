...
The code now refuses unrelated private direct routes/DNS servers and suffix text collisions; provisions credentials only through the installed manifest-pinned provisioner's stdin pipe before agent start; derives setup/uninstall file checks from every signed payload entry and rejects unexpected owned-root files; and requires locally reviewed production server service/config/listener/core ownership. Integrated tests mutate each of those bindings to prove the corresponding false pass is rejected.
